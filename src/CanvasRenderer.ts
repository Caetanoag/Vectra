...
export class CanvasRenderer {
	 ...
	 * ```
	 */
	public fillCircle(circle: Circle, color: Color): void;
	public fillCircle(center: Vector2, radius: number, color: Color): void;
	public fillCircle(
		center: Vector2 | Circle,
		radius: number | Color,
		color?: Color,
	): void {
		const circle =
			center instanceof Circle ? center : new Circle(center, radius as number);
		const fillColor = center instanceof Circle ? (radius as Color) : color;
		if (!fillColor) throw new Error("Color is required.");
		this.context.save();
	 ...
	 * ```
	 */
	public strokeCircle(circle: Circle, color: Color, lineWidth?: number): void;
	public strokeCircle(
		center: Vector2,
		radius: number,
		color: Color,
	...
	public strokeCircle(
		...
		const circle =
			center instanceof Circle ? center : new Circle(center, radius as number);
		const strokeColor = center instanceof Circle ? (radius as Color) : color;
