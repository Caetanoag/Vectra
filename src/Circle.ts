...
export class Circle {
	...
	constructor(center: Vector2, radius: number) {
		if (radius < 0 || !Number.isFinite(radius))
			throw new Error(
				"Radius must be a finite number greater or equal to zero",
			);
		this.center = center;
		this.radius = radius;
	}
	 ...
	 * ```
	 */
	public setRadius(radius: number): Circle {
		this.radius = radius;
		return this;
	}
	/**
	 ...
	 * ```
	 */
	public setCenter(center: Vector2): Circle {
		this.center = center;
		return this;
	}
	/**
	 ...
	 * ```
	 */
	public get equationString(): string {
		const signal = (n: number): string => {
			if (n <= 0) {
				return "+";
			}
			return "-";
		};
		return `(x ${signal(this.center.x)} ${Math.abs(this.center.x)})^2 + (y ${signal(this.center.y)} ${Math.abs(this.center.y)})^2 = ${this.radius * this.radius}`;
	}
	/**
	 ...
	 * ```
	 */
	public containsPoint(point: Vector2, epsilon: number = 1e-9): boolean {
	 ...
	 * ```
	 */
	public distanceTo(other: Vector2 | Circle) {
	 ...
	 * ```
	 */
	public equals(other: Circle, epsilon: number = 1e-9): boolean {
		return (
	 ...
	 * ```
	 */
	public pointAt(angle: number): Vector2 {
	 ...
	 * ```
	 */
	public intersects(other: Circle, epsilon: number = 1e-9): boolean {
	 ...
	 * ```
	 */
	public containsCircle(other: Circle, epsilon: number = 1e-9): boolean {
		const distance = this.center.distanceTo(other.center);
	 ...
	 * ```
	 */
	public containsBox(box: Rect, epsilon: number = 1e-9): boolean {
		const corners = [
	 ...
	 * ```
	 */
	public withCenter(center: Vector2): Circle {
	 ...
	 * ```
	 */
	public withRadius(radius: number): Circle {
