import { CanvasRenderer, Color, Rect, } from "https://cdn.jsdelivr.net/gh/reinhackVancheat/Vectra/lib/index.js";
const canvas = document.getElementById("canvas");
const renderer = new CanvasRenderer(canvas);
renderer.setSize(window.innerWidth, window.innerHeight);
const size = Math.min(canvas.width, canvas.height) * 0.1;
const rect = new Rect((canvas.width - size) / 2, (canvas.height - size) / 2, size, size);
let speed = 2;
function loop() {
